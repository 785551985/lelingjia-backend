...
public class DeepSeekAdapter implements LlmAdapter {
    ...
    public DeepSeekAdapter(String apiUrl, String apiKey, String model) {
            ...
            return webClient.post()
                    ...
                    .flatMap(line -> {
                        ...
                        try {
                            ...
                            log.error("解析 DeepSeek 流式响应失败, line: {}", line, e);
                        }
                        return Flux.empty();
                    });
        } catch (Exception e) {
    ...
    public String chat(String systemPrompt, String userMessage, List<Map<String, String>> history) {
        try {
            ...
            log.error("同步调用 DeepSeek 失败", e);
            throw new RuntimeException("调用 DeepSeek 服务失败: " + e.getMessage(), e);
        }
    }
